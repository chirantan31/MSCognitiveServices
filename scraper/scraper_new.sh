...
for url in ${urls[@]}
do
    wget --header="Cookie: CloudFront-Key-Pair-Id=${args[0]}; CloudFront-Policy=${args[1]}; CloudFront-Signature=${args[2]}" --header="Connection: keep-alive" "${url}" -O "audio${n}.mp3" -c
    ((n+=1))
done
