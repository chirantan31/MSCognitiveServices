...
for url in ${urls[@]}
do
    wget --header="Cookie: CloudFront-Key-Pair-Id=${args[1]}; CloudFront-Policy=${args[2]}; CloudFront-Signature=${args[3]}" --header="Connection: keep-alive" "${url}" -O "audio${n}.mp3" -c
    ((n+=1))
done
