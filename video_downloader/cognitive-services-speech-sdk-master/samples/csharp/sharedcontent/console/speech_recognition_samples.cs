...
namespace MicrosoftSpeechSDKSamples
{
    public class SpeechRecognitionSamples
    {
        ...
        public static async Task ContinuousRecognitionWithFileAsync()
        {
            ...
            using (var audioInput = AudioConfig.FromWavFileInput(@"Test4_ML2.wav"))
            {
        ...
        public static async Task RecognitionWithAudioStreamAsync(String file)
        {
                ...
                using (var recognizer = new SpeechRecognizer(config, audioInput))
                {
                    ...
                    recognizer.Recognized += (s, e) =>
                    {
                        if (e.Result.Reason == ResultReason.RecognizedSpeech)
                        {
                            // Console.WriteLine($"RECOGNIZED: Text={e.Result.Text}");
                            /// int duration = 
                            // Console.WriteLine($"Duration={e.Result.Duration.Minutes}:{e.Result.Duration.Seconds},{e.Result.Duration.Milliseconds}");
                            // Console.WriteLine($"OffsetMillis={offsetMillis}");
                            ...
                            Console.WriteLine(sub.Subtitle());
                            try
                            {
